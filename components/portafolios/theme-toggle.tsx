"use client";

import { usePortfolioTheme } from "@/lib/portafolios/theme-context";

export function ThemeToggle() {
  const { theme, toggle } = usePortfolioTheme();
  const isDark = theme === "dark";

  return (
    <button
      type="button"
      onClick={toggle}
      aria-label={isDark ? "Cambiar a modo claro" : "Cambiar a modo oscuro"}
      className="relative flex items-center w-11 h-6 rounded-full transition-colors duration-300 focus:outline-none"
      style={{
        backgroundColor: isDark ? "#27272a" : "#e4e4e7",
      }}
    >
      {/* Sun icon */}
      <span
        className={`absolute left-1 text-[10px] transition-opacity duration-200 ${
          isDark ? "opacity-30" : "opacity-100"
        }`}
      >
        <svg className="w-3.5 h-3.5 text-amber-500" fill="currentColor" viewBox="0 0 20 20">
          <path d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" />
        </svg>
      </span>

      {/* Moon icon */}
      <span
        className={`absolute right-1 text-[10px] transition-opacity duration-200 ${
          isDark ? "opacity-100" : "opacity-30"
        }`}
      >
        <svg className="w-3.5 h-3.5 text-blue-300" fill="currentColor" viewBox="0 0 20 20">
          <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
        </svg>
      </span>

      {/* Dot */}
      <span
        className="absolute w-4 h-4 rounded-full shadow-sm transition-all duration-300"
        style={{
          backgroundColor: isDark ? "#fafafa" : "#ffffff",
          left: isDark ? "calc(100% - 20px)" : "2px",
        }}
      />
    </button>
  );
}
